...
| Path | Holds |
|------|-------|
| `src/app/` | `App.tsx` — strings provider + Practice/Collection mode state; `CollectionScreen` (import UI, thumbnails-only grid, storage gauge, Select-mode bulk tag assign, per-item sheet, tag manager); placeholder `PracticeScreen`. |
| `src/components/` | `ModeToggle` (mode switcher); `SourceSheet` (per-item bottom sheet: preview, size, tag chips, create-and-assign, delete); `TagAssignPanel` (tag chips for the current selection, mixed-state aware); `TagManager` (rename + confirmed-delete tag list); `ConfirmDialog` (confirm dialog on `Sheet`); `primitives/Sheet` (native `<dialog>` shell — bottom sheet on mobile, centered card on desktop, ported from HRV), `primitives/PageShell` (width-capped page frame). |
...
| `src/storage/` | `db.ts` (IndexedDB `cute-db` **v3**: `sources`/`blobs`/`thumbs`/`sessions`/`holdEvents`/`tags` behind `Result` wrappers, atomic `writeMany`; media stored as ArrayBuffer+MIME, not Blob; v2 rung seeds tags, v3 rung clears media stores), `tags.ts` (create/rename/delete/assign — sources reference tags by id, delete strips the id from every source in one tx), `sources.ts` (tombstone delete: blob+thumb removed, record kept), `prefs.ts` (coerce-and-fallback prefs), `storage.ts` (localStorage envelope `cute:state:v1`), `persistence.ts` (`navigator.storage.persist()`). Barrel `index.ts`. |
