// Video import pipeline (SPEC FR-9): prove the browser can decode the file by
// loading it into an in-document <video>, capture a poster frame as the
...
// HTMLMediaElement.readyState: a current frame is decodable at HAVE_CURRENT_DATA.
...
async function probeVideo(blob: Blob): Promise<ImageBitmap> {
  ...
  // element never loads. Park it in-viewport but invisible; off-screen or
  // display:none can also suppress the decode.
  video.style.cssText = 'position:fixed;top:0;left:0;width:1px;height:1px;opacity:0;pointer-events:none'
  document.body.appendChild(video)
  try {
    video.src = url
    // Kick iOS's decode, but do NOT await play(): on iOS the frame loads and
    // fires `loadeddata` while play() is still pending, so a listener attached
    // afterward misses the event and the wait times out. Fire play, then gate
    // on readyState — proceed if a frame already arrived, else await the event.
    void video.play().catch(() => {})
    if (video.readyState < HAVE_CURRENT_DATA) await videoEvent(video, 'loadeddata')
    ...
    if (Number.isFinite(video.duration) && video.duration > POSTER_SEEK_S) {
      // Listen before moving currentTime so a fast seek can't fire first;
      // best-effort — a stalled seek keeps the loaded frame over losing the video.
